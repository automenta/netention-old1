package automenta.netention.owl;
...
public class ImportOWL {
    ...
    public void printHierarchy(OWLReasoner reasoner, OWLClass clazz, int level)
        ...
        if (reasoner.isSatisfiable(clazz)) {
            ...
            Pattern t = self.getPattern(clazz.getIRI().getFragment());
            if (t == null) {
                try {
                    t = new Pattern(clazz.getIRI().toString(), clazz.getIRI().getFragment());
                    self.addPattern(t);
    ...
    public static void main(String[] args) {
        ...
        try {
           ...
           //self.saveJSON("/tmp/netention.json");
        } catch (Exception ex) {
            ex.printStackTrace();
        }
    }
